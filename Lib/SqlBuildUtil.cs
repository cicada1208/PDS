...
namespace Lib
{
    public class SqlBuildUtil
    {
        ...
        /// <param name="entity">model entity, model 未必定義 TableAttribute</param>
        public virtual SqlCmdUtil Select(Type type, dynamic entity = null, bool schemaOnly = false)
        {
            ...
            if (!schemaOnly)
            {
                var props = type.GetProperties(BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.Instance);  // 排除繼承屬性
                ...
                ((entity as object)?.GetType()?.GetProperty(p.Name)?.GetValue((entity as object))).NullableToStr() != string.Empty) // p.GetValue(entity)
