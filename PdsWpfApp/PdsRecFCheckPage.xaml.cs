...
{
    /// <summary>
    /// PdsRecFCheckPage.xaml 的互動邏輯
    /// </summary>
    public partial class PdsRecFCheckPage : Page
    {
        public PdsRecFCheckPage()
        {
            InitializeComponent();
            PdsRecFCheckViewModel.ShowPdsRecCancelWindow = ShowPdsRecCancelWindow;
            PdsRecFCheckViewModel.ShowAtcCodeGroupWindow = ShowAtcCodeGroupWindow;
            PdsRecFCheckViewModel.ShowLstudWindow = ShowLstudWindow;
            PdsRecFCheckViewModel.ShowPdsNoteEditWindow = ShowPdsNoteEditWindow;
            this.Loaded += PdsRecFCheck_Loaded;
        }

        private void PdsRecFCheck_Loaded(object sender, System.Windows.RoutedEventArgs e)
        {
            if (PdsRecFCheckViewModel.LstCodeEnabled)
                this.LstCodeTextBox.Focus();
        }

        private PdsRecFCheckViewModel PdsRecFCheckViewModel =>
            this.DataContext as PdsRecFCheckViewModel;
